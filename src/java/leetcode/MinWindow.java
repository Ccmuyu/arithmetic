...
public class MinWindow {
    ...
    public static void main(String[] args) {
        ...
        String s = window.minWindow("ADOBECODEBANC", "ABC");
    ...
    public String minWindow(String s, String t) {
        ...
        while (r < s.length()) {
            char c = s.charAt(r);
            if (need[c] > 0) {  //表示t中包含当前遍历到的这个c字符，更新目前所需要的count数大小，应该减少一个
                count--;
            }
            //无论这个字符是否包含在t中，need[]数组中对应那个字符的计数都减少1，利用正负区分这个字符是多余的还是有用的
            need[c]--;
            if (count == 0) {  //count==0说明当前的窗口已经满足了包含t所需所有字符的条件
                while (l < r && need[s.charAt(l)] < 0) {//如果左边界这个字符对应的值在need[]数组中小于0，说明他是一个多余元素，不包含在t内
                    need[s.charAt(l)]++;    //在need[]数组中维护更新这个值，增加1;因为上面-1
                    l++;                    //左边界向右移，过滤掉这个元素
                }
                if (r - l + 1 < size) {//如果当前的这个窗口值比之前维护的窗口值更小，需要进行更新
                    size = r - l + 1; //更新窗口值
                    start = l; //更新窗口起始位置，方便之后找到这个位置返回结果
                }
                need[s.charAt(l)]++; //先将l位置的字符计数重新加1
                l++;                //重新维护左边界值和当前所需字符的值count
                count++;
            }
            //右移边界，开始下一次循环
            r++;
